...
library(collapse)
library(SmartEDA)
library(tidyverse) # El paquete `tidyverse` es una coleccion de paquetes que sirven para manejar, modificar y visualizar datos
```
...
[Volver arriba](Estadistica_descriptiva.html)
...
Por defecto R aplica la fórmula para calcular la varianza muestral con el denominador \(n - 1\):
...
Al trabajar con una población completa es correcto utilizar la varianza poblacional. Para calcularla debemos modificar manualmente el factor de corrección multiplicando por 
\(\frac{(n - 1)}{n}\) para pasar a denominador \(N\):

```{r}
var(datos$altura) * (length(datos$altura) - 1) / length(datos$altura)
```

También podemos escribir la fórmula de la varianza poblacional directamente:

```{r}
mean((datos$altura - mean(datos$altura))^2)
```
...
De igual forma que con la varianza, R calcula de forma automática la desviación estándar muestral, esto porque produce estimadores insesgados y es estándar en estadística inferencial:
...
Como la desviación estándar es la raíz cuadrada de la varianza, para calcular la desviación poblacional corregimos el factor multiplicando por \(\sqrt{\frac{(n - 1)}{n}}\) para pasar a denominador \(N\):

```{r}
sd(datos$altura) * sqrt((length(datos$altura) - 1) / length(datos$altura))
```

También podemos aplicar la raíz cuadrada directamente a la fórmula de la varianza:

```{r}
sqrt(mean((datos$altura - mean(datos$altura))^2))
```
...
En concreto, la covarianza indica la dirección de la relación entre dos variables:

- Covarianza positiva: cuando una variable aumenta, la otra tiende a aumentar.

- Covarianza negativa: cuando una variable aumenta, la otra tiende a disminuir.

- Covarianza cercana a cero: no hay una relación lineal clara entre ellas.

En la práctica la covarianza se utiliza para explorar relaciones entre variables antes de análisis más formales. 
Es la base para calcular la correlación (Pearson).
En biología y ecología, ayuda a ver si dos rasgos o variables ambientales cambian conjuntamente.
...
La covarianza depende de las unidades de medida, por lo que su valor no es fácil de interpretar por sí solo. Por eso, muchas veces se prefiere la correlación, que está estandarizada entre −1 y 1
...
En R podemos utilizar varias funciones que nos brindan un resumen de nuestras tablas de datos, que incluyen varias de las medidas antes vistas.

La función `summary()` del paquete base de R muestra una descripción rápida de los datos, con la cantidad de observaciones para las variables categóricas, y medidas básicas para las variables numéricas:

```{r}
summary(datos)
```

La función `skim()` del paquete `skimr` hace algo similar, pero separa la información de la tabla, las variables categóricas y las variables numéricas en pestañas diferentes:

```{r}
skim(datos)
```

La función `describe()` del paquete `psych` nos muestra un resumen de nuestras variables numéricas con algunas medidas de tendencia central y medidas de dispersión.
...
La función `descr()` del paquete `collapse` hace lo mismo que la función `describe()` pero nos muestra todas las variables por separado, no en una tabla:
...
También podemos explorar nuestras bases de datos con diferentes funciones del paquete `SmartEDA` que está dedicado al "Exploratory Data Analysis":

```{r}
ExpData(datos, type = 1)
```

```{r}
ExpData(datos, type = 2)
```

```{r}
ExpData(datos, type = 2, fun = c("mean", "median", "sd"))
```

```{r}
ExpNumStat(datos, Outlier = TRUE)
```

Si queremos ver un resumen descriptivo de nuestras variables numéricas agrupadas de acuerdo a una variable categórica, podemos utilizar la función `Summarize()` del paquete `FSA`:

```{r}
Summarize(altura ~ sexo, data = datos)
```
