...
Entre los datos básicos tenemos:
...
```{r}
d <- as.Date("26-01-26", format = "%d-%m-%y")
class(d)
```
...
```{r}
df <- data.frame(especie = c("A", "B"), altura = c(10, 15))
class(df)
```
...
Si el número de observaciones que colocamos supera el rango definido debemos agregar el argumento `replace = TRUE` para permitir que se repitan diferentes observaciones:

```{r}
altura4 <- sample(150:190, size = 100, replace = TRUE)
table(altura4)
```
