...
```{r}
data("penguins")
penguins <- drop_na(penguins, flipper_length_mm)
head(penguins)
```

<br>

[Volver arriba](ANOVA.html)
...
En la primera columna del output encontramos las fuentes de variación: el **factor** (tratamiento o variable explicativa) y los **Residuals** que es la variación no explicada (error).

La columna **Df** (degrees of freedom) indica los grados de libertad de cada fuente de variación (n-1).

La Suma de Cuadrados (**Sum Sq**) cuantifica la variabilidad total asociada a cada fuente.

El Cuadrado Medio (**Mean Sq**) es la suma de cuadrados dividida entre los grados de libertad

El Estadístico F (**F value**) es la razón entre el Mean Sq del factor y el Mean Sq del error.

Un valor F alto indica que la variación entre grupos es mucho mayor que la variación dentro de los grupos.

El valor de p (Pr(>F)) indica la probabilidad de observar un valor F igual o mayor al obtenido si la hipótesis nula fuera cierta. 

- Si el valor de **p < 0.05**, la diferencia entre la varianza de las medias entre al menos dos grupos es estadísticamente significativa.
...
Para realizar gráficos más elaborados utilizamos el paquete `ggplot2`.
...
El IC95% se calcula multiplicando el error estándar por el _valor Z_ para ese nivel de confianza.
...
También podemos calcular las medias y los intervalos de confianza con la función `ggpredict()` del paquete `ggeffects`:

```{r}
pred <- ggpredict(anova, terms = "species")
ggplot(pred, aes(x, predicted)) +
  ...
  labs(y = "Longitud de aleta (mm)",
       x = "Especie") +
  geom_signif(data = penguins, aes(species, flipper_length_mm),
              comparisons = list(c("Adelie", "Chinstrap"),
                                 c("Adelie", "Gentoo"),
                                 c("Chinstrap", "Gentoo")),
              size = 0.5,
              map_signif_level = TRUE,
              y_position = c(196,217,220),
              tip_length = 0.01) +
  theme_classic()
```
...
ggplot(sum2vias, aes(species, mean)) + 
  geom_errorbar(aes(color = island, # indicamos que el color de las barras de error sea en base a la isla (segunda variable predictora)
                    ymin = liIC95, 
                    ymax = lsIC95),
                width = 0.05, 
  ...
  geom_point(aes(color = island),
             shape = 19, 
             size = 2,
             position = position_dodge(width = 0.3)) + # indicamos que el color de los puntos sea en base a la isla
labs(y = "Longitud de aleta (mm)",
     x = "Especie",
     color = "Isla") +
