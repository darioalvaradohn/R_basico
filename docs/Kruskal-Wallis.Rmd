...
[Volver arriba](Kruskal-Wallis.html)
...
```{r}
k <- kruskal(penguins$body_mass_g, penguins$species)
k
```
...
```{r}
sumbm <- Summarize(body_mass_g ~ species, data = penguins)
sumbm
```
...
ggplot(sumbm, aes(species, mean)) +
  ...
  geom_point(shape = 19, 
             size = 3, 
             color = c("gray", "lightblue", "darkslategray")) +
  labs(title = "Diferencias entre la masa corporal de pingüinos por especie",
       y = "Masa corporal (g)",
       x = "Especie") +
  theme_classic() +
  geom_signif(data = penguins, aes(species, body_mass_g),
              comparisons = list(c("Adelie", "Chinstrap"),
                                 c("Adelie", "Gentoo"),
                                 c("Chinstrap", "Gentoo")),
              size = 0.5,
              map_signif_level = TRUE,
              y_position = c(3800,5300,5100),
              tip_length = 0.01)
```
